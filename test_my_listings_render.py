...
# Create dummy routes so url_for works
@app.route('/')
def home():
    return 'Home'
...
with app.app_context():
    try:
        ...
        # Verify key elements
        checks = [
            ("Active tab", "Active (1)" in html),
            ("Expired tab", "Expired (1)" in html),
            ("Sold tab", "Sold (1)" in html),
            ("iPhone title", "iPhone 14 Pro" in html),
            ("Samsung title", "Samsung Galaxy S21" in html),
            ("OnePlus title", "OnePlus 11" in html),
            ("Boost button", "Boost" in html),
            ("View buttons", "View" in html),
            ("Mark Sold button", "Mark Sold" in html),
        ]
